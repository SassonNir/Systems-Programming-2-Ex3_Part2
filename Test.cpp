...
std::vector<double> getRandVecMat(int size);
...
TEST_CASE("Matrices dimension isn't the same") {
    ...
    std::vector<double> randVecMat = getRandVecMat(row * col);
    std::vector<double> randVecMat2 = getRandVecMat(row2 * col2);
...
TEST_CASE("Comparison Operators") {
    ...
    int col = 1 + rand() % 100;
    std::vector<double> randVecMat = getRandVecMat(row * col);
    std::vector<double> OtherRandVecMat = getRandVecMat(row * col);

    /* Sum each matrix */
    double sumA = 0;
    double sumB = 0;
    for (double &element : randVecMat) {
        sumA += element;
    }
    for (double &element : OtherRandVecMat) {
        sumB += element;
    }
...
TEST_CASE("Increment - Decrement Matrix") {
    ...
    std::vector<double> randVecMatCopy(randVecMat);
    Matrix expectedRandMat{randVecMatCopy, row, col};

    /* Check postfix '--' operation */
    CHECK((randMat-- == expectedRandMat));
    for (double &element : randVecMatCopy) {
        element--;
    }
...
TEST_CASE("Constructor") {
    ...
    std::vector<double> randVecMat = getRandVecMat(10);
